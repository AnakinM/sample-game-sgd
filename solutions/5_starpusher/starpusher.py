...
def main():
    ...
    global BASICFONT, PLAYERIMAGES, currentImage
...
def readLevelsFile(filename):
    ...
    for lineNum in range(len(content)):
        ...
        if line != "":
            ...
            assert (
                startx is not None and starty is not None
            ), """Level %s (around line %s) in %s is missing a "@" or "+" to mark the
                start point.""" % (
                levelNum + 1,
                lineNum,
                filename,
            ...
            assert len(stars) >= len(
                goals
            ), """Level %s (around line %s) in %s is impossible to solve. It has %s
                goals but only %s stars.""" % (
                levelNum + 1,
                lineNum,
                filename,
                len(goals),
